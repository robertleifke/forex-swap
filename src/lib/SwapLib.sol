...
library SwapLib {
    ...
    function computeLnSDivK(uint256 S, uint256 strike_) public pure returns (int256) {
        int256 ratio = int256(S.divWadDown(strike_));
        return FixedPointMathLib.lnWad(ratio);
    }

    /// @dev Computes σ√τ given `sigma_` σ and `tau` τ.
    function computeSigmaSqrtTau(uint256 sigma_, uint256 tau_) public pure returns (uint256) {
    ...
    function computeLGivenX(
        ...
        int256 lnSDivK = computeLnSDivK(amount0, strike);
        uint256 sigmaSqrtTau = computeSigmaSqrtTau(sigma, timeToMaturity);
        uint256 halfSigmaSquaredTau = sigma.mulWadDown(sigma).mulWadDown(0.5 ether).mulWadDown(timeToMaturity);
        int256 d1 = 1 ether * (lnSDivK + int256(halfSigmaSquaredTau)) / int256(sigmaSqrtTau);
        uint256 cdf = uint256(Gaussian.cdf(d1));

        return amount0.divWadUp(1 ether - cdf);
    }

    /// @dev x is independent variable, y and L are dependent variables.
    function findX(bytes memory data, uint256 x) public pure returns (int256) {
    ...
    function findY(bytes memory data, uint256 y) public pure returns (int256) {
    ...
    function findL(bytes memory data, uint256 liquidity) public pure returns (int256) {
    ...
    function sum(uint256 a, int256 b) public pure returns (uint256) {
        if (b < 0) {
            if (a >= uint256(-b)) {
                return a - uint256(-b);
            } else {
    ...
    function upscale(uint256 amount, uint256 scalingFactor) public pure returns (uint256) {
    ...
    function downscaleDown(uint256 amount, uint256 scalar_) public pure returns (uint256) {
        return FixedPointMathLib.divWadDown(amount, scalar_);
    }

    /// @dev Converts a WAD amount to a native DECIMAL amount, rounding up.
    function downscaleUp(uint256 amount, uint256 scalar_) public pure returns (uint256) {
    ...
    function scalar(address token) public view returns (uint256) {
        uint256 decimals = ERC20(token).decimals();
        uint256 difference = 18 - decimals;
        return FixedPointMathLib.WAD * 10 ** difference;
    }

    function isASmallerApproxB(uint256 a, uint256 b, uint256 eps) public pure returns (bool) {
